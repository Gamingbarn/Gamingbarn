tag @p[tag=player,limit=1] add slime
...
execute at @a[limit=1,sort=random] run summon slime ~ ~200 ~ {NoGravity:1b,Invulnerable:1b,PersistenceRequired:1b,NoAI:1b,CanPickUpLoot:0b, Tags:["slime","p1"],Size:1}
...
kill @e[type=dolphin,tag=dolphin,tag=p1]
kill @e[type=creeper,tag=creeper,tag=p1]
