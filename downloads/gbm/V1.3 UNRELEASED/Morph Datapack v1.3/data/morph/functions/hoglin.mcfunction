tag @p[tag=player,limit=1] add hoglin
...
tag @p[tag=player,limit=1] remove bee

execute at @a[limit=1,sort=random] run summon hoglin ~ ~200 ~ {NoGravity:1b,Invulnerable:1b,PersistenceRequired:1b,NoAI:1b,CanPickUpLoot:0b,Age:-1000000000,AgeLocked:1b,IsImmuneToZombification:1b,CannotBeHunted:1b,Tags:["hoglin","p1"]}
...
execute at @p[sort=random] run tp @e[type=bee,tag=bee] ~ ~-200 ~
...
kill @e[type=bee,tag=bee,tag=p1]
